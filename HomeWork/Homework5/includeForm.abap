...
*Öğrenci başarıyla silindiyse tabloyu güncelle
      ...
      WHEN '&EXC'.
        ...
        DATA: lt_excel_template TYPE TABLE OF ty_t_excel_template,
              ls_excel_template TYPE ty_t_excel_template.
...
*  Put Excel in front.
        ...
        SET PROPERTY OF cell 'VALUE' = 'Öğrenci ID'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'B1'.
        SET PROPERTY OF cell 'VALUE' = 'Öğrenci Adı'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'C1'.
        SET PROPERTY OF cell 'VALUE' = 'Öğrenci Soyadı'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'D1'.
        SET PROPERTY OF cell 'VALUE' = 'Ders ID'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'E1'.
        SET PROPERTY OF cell 'VALUE' = 'Vize'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'F1'.
        SET PROPERTY OF cell 'VALUE' = 'Final'.

        CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'G1'.
        SET PROPERTY OF cell 'VALUE' = 'Başarı Durumu'.


        LOOP AT gt_list ASSIGNING FIELD-SYMBOL(<fs_list>).
          ls_excel_template-ogr_id   = <fs_list>-ogr_id.
          ls_excel_template-ogr_ad   = <fs_list>-ogr_ad.
          ls_excel_template-ogr_syd  = <fs_list>-ogr_syd.
          ls_excel_template-les_name = <fs_list>-les_name.
          ls_excel_template-note1    = <fs_list>-note1.
          ls_excel_template-note2    = <fs_list>-note2.
          ls_excel_template-durum    = <fs_list>-durum.
          APPEND ls_excel_template TO lt_excel_template.
        ENDLOOP.
        " Append Excel Sample Data to Internal Table.
        LOOP AT lt_excel_template INTO ls_excel_template.
          ...
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-ogr_id NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'B2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-ogr_ad NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'C2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-ogr_syd NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'D2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-les_name NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'E2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-note1 NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'F2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-note2 NO FLUSH.

          CALL METHOD OF excel 'RANGE' = cell EXPORTING #1 = 'G2'.
          SET PROPERTY OF cell 'VALUE' = ls_excel_template-durum NO FLUSH.
